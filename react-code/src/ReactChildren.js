...
/**
 * Traverses children that are typically specified as `props.children`, but might also be specified through attributes:
 * 遍历儿童，通常被指定为`props.children`。，但也可以通过属性指定:
 *
 * - `traverseAllChildren(this.props.children, ...)`
 * - `traverseAllChildren(this.props.leftPanelChildren, ...)`
 *
 * The `traverseContext` is an optional argument that is passed through the entire traversal. 
 * It can be used to store accumulations or anything else that the callback might find relevant.
 * “traverseContext”是一个可选的参数，它在整个遍历中传递。
 * 它可以用于存储累积或任何其他回调可能会发现相关的内容。
 *
...
/**
 * map into with key prefix internal
 * 映射到具有关键前缀的内部
 * 
 * @param {*} children 
 * @param {*} array 
 ...
 * @param {*} context 
 * 
 * toArray -> mapIntoWithKeyPrefixInternal(children, result, null, child => child);
 */
 ...
 * 计算通常被指定为“props.children”的孩子的数量。
 *
